...
The existing paperless-ngx instance is fully migrated, searchable, and accessible via both the web UI and MCP. Imported documents retain their original Paperless added date, while all other metadata is regenerated by Docs-AI. A single `docker compose up` starts the entire application. Search feels responsive and results are relevant.
